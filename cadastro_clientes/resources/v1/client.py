...
def save_client(client_data: ClientInsert = Body(..., example=ClientInsert.Config.schema_extra)):
    """
    ...
    """
    return {"result": [cl.insert(client_data.dict(by_alias=True))]}
...
def find_by_region_and_type(request: Request,
    ...
    filtered_client_fields = [ClientShow(**client).dict(exclude={"id", "collection"}) for client in clients]
...
def find_by_id(collection: str = Path(..., description="Coleção do cliente buscado"),
    ...
    return {"result": [ClientShow(**cl.find_by_id(collection, client_id)).dict(exclude={"id", "collection"})]}
